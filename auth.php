...
<head>
  ...
  <style>
    ...
    .hidden-tab { opacity: 0; transform: translateX(20px); pointer-events: none; position: absolute; }
    .visible-tab { opacity: 1; transform: translateX(0); position: relative; }
    ...
    .popup.active { display: flex; animation: fadeIn 0.3s ease forwards; }
    .popup-content {
      background: white; padding: 2.5rem 3rem; border-radius: 20px;
      text-align: center; transform: scale(0.8); opacity: 0;
      animation: popIn 0.4s ease forwards; box-shadow: 0 10px 40px rgba(0,0,0,0.25);
    }
    ...
    .checkmark { border: 4px solid #22c55e; box-shadow: 0 0 15px rgba(34,197,94,0.3); }
    .checkmark::after {
      content: ""; position: absolute; width: 25px; height: 45px;
      border-right: 5px solid #22c55e; border-bottom: 5px solid #22c55e;
      transform: rotate(45deg) scale(0); animation: checkDraw 0.4s 0.3s ease forwards;
    }
    .crossmark { border: 4px solid #ef4444; box-shadow: 0 0 15px rgba(239,68,68,0.3); }
    .crossmark::before, .crossmark::after {
      content: ""; position: absolute; width: 45px; height: 5px;
      background: #ef4444; border-radius: 5px; transform: scale(0);
    ...
    @keyframes checkDraw { to { transform: rotate(45deg) scale(1); } }
    @keyframes crossDraw { to { transform: scale(1); } }
...
<body class="bg-blue-50 flex items-center justify-center min-h-screen">
  ...
  <!-- ✅ Success Popup -->
  <div class="popup <?php if($showSuccessPopup) echo 'active'; ?>" id="successPopup">
    <div class="popup-content">
      <div class="checkmark"></div>
      <h2 class="text-2xl font-semibold text-green-600 mb-2">Success!</h2>
      <p class="text-gray-600">
        <?= $registered ? 'Account created successfully. You can now login!' : 'Welcome back! Redirecting to dashboard...' ?>
      </p>
    </div>
  </div>
  ...
  <script>
    ...
    // ✅ Redirect to main.php after login success
    window.addEventListener('DOMContentLoaded', () => {
      switchMode('<?= $mode ?>');
      const popup = document.getElementById('successPopup');
      if (popup && popup.classList.contains('active') && <?= $success ? 'true' : 'false' ?>) {
        setTimeout(() => {
